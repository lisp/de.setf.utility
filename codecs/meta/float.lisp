         ...
         (ecase *read-default-float-format*
           ...
           ((short-float single-float)
            (if (plusp es)
              ...
              (when (> e 45)
                (return-from meta:parse-float (if (plusp s) 0.0s0 -0.0s0))))))
           ((double-float long-float rational)
            ;; treat default setting for rational as double float
            (if (plusp es)
              (when (> e 308)
                (if (plusp s)
              ...
              (when (> e 324)
                (return-from meta:parse-float (if (plusp s) 0.0d0 -0.0d0))))))
